...
local puckEffects = {
	["sine"] = function(y)
		return Vector3.new(0, 0, math.sin(y / 10) / 2) -- slidy
	end,
	["cos"] = function(y)
		return Vector3.new(math.sin((y * 1.25) / 5) / 20, 0, 0) -- wavy
	end,
	["sinecos"] = function(y)
		return Vector3.new(math.sin((y * 1.25) / 5) / 20, 0, 0) + Vector3.new(0, 0, math.sin(y / 10) / 2)
	end,
}
		...
		coroutine.wrap(function()
			...
			while true do
				...
				local b = (bt:FindFirstChild("effect") and puckEffects[bt.effect.Value])
						and puckEffects[bt.effect.Value](y)
					or Vector3.new()

				puck.CFrame = CFrame.new(
					puck.Position.X,
					102.875,
					-2 + y / 20 * (puck:FindFirstChild("FAST") and speed * 2 or speed)
				) + b
				if bt:FindFirstChild("effect") and bt.effect.Value == "ghost" then
					puck.Transparency = puckAppearances["ghost"]["Transparency"] * (y / 12)
				end
			end
...
local function attack(sp)
	...
	if sp then
		...
		while true do
			...
			r.CFrame = r.CFrame + Vector3.new(vx * (dt * 60), 0, vy * (dt * 60))
			...
			BTT.opp.CFrame = BTT.opp.CFrame:Lerp(
				CFrame.new(r.Position.X + (math.sin(tick()) + math.random(-100, 100) / 100) / 100, 103.034, -4),
				1 - 0.021 ^ dt
			)
		end
		...
		for x = 1, 100 do
			...
			moveFactorThing += (dt * 60)
			r.CFrame = CFrame.new(moveFactorThing / 30 * dr - dr, 102.875, 3)
		...
		if miss == 0 or not miss then
			...
			for x = 1, 5 do
				local dt = RunService.RenderStepped:Wait()
				moveFactorThing += (dt * 60)
				BTT.you.CFrame += Vector3.new(0, 0, -moveFactorThing / 50)
		...
		if miss then
			...
			for x = 1, 200 do
				...
				r.CFrame = r.CFrame + Vector3.new(bx * (dt * 60), 0, -(dt * 60) / 2)
